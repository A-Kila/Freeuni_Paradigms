...
(define (partition pivot num-list less?)
  (if (null? num-list) '(() ())
      (let ((split-of-rest (partition pivot (cdr num-list) less?)))
	(if (less? (car num-list) pivot)
...
(define (quicksort num-list less?)
  (if (<= (length num-list) 1) num-list
      (let ((split (partition (car num-list) (cdr num-list) less?)))
	(append (quicksort (car split) less?) 
		(list (car num-list)) 
		(quicksort (cadr split) less?)))))
...
(define (intersection-points circles)
  (define (helper rest)
    (if (null? rest) '()
      (append (intersect (car circles) (car rest)) 
        (helper (cdr rest)))
    )
  )
  (if (null? circles) '()
    (append (helper (cdr circles))
...
(define (sort-points points) 
  (quicksort points (lambda (p1 p2) (< (car p1) (car p2))))
)

(define (clumped-points points) 
  (let ((sorted-points (map cadr (sort-points (rate-points points))))) 
    (prefix-of-list sorted-points (quotient (length points) 2))
  )
)
...
(define (average-point points) 
  (let ((average 
          (map (lambda (x) (/ x (length points))) 
            (list (apply + (map x points)) (apply + (map y points)))
          )
       ))
